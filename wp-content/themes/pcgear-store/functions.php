...
function pcgear_store_header_navigation() {
	...
	$guide_cpu_url        = pcgear_store_page_url('huong-dan-chon-cpu-mainboard', $builder_url);
	$guide_ram_url        = pcgear_store_page_url('huong-dan-socket-va-ram', $builder_url);
	$guide_psu_url        = pcgear_store_page_url('huong-dan-cong-suat-nguon', home_url('/product-category/psu/'));
	$guide_checkout_url   = pcgear_store_page_url('huong-dan-gio-hang-thanh-toan', $cart_url);
	$guide_flow_url       = pcgear_store_page_url('flow-demo-bccd', $builder_url);
	$software_intro_url   = pcgear_store_page_url('giai-thich-pc-builder', $builder_url);
	$software_rules_url   = pcgear_store_page_url('kiem-tra-tuong-thich', $builder_url);
	...
	return array(
		...
		array(
			...
			'groups' => array(
				array(
					'type'  => 'card',
					'title' => __('Trung tâm hướng dẫn', 'pcgear-store'),
					'text'  => __('Tổng hợp luồng demo, cách build PC và cách trình bày plugin khi bảo vệ BCCĐ.', 'pcgear-store'),
					'link'  => $guide_flow_url,
					'cta'   => __('Xem tất cả', 'pcgear-store'),
				),
				array(
					'type'  => 'list',
					'title' => __('Build và tương thích', 'pcgear-store'),
					'links' => array(
						array('label' => __('Cách chọn CPU và mainboard', 'pcgear-store'), 'url' => $guide_cpu_url),
						array('label' => __('Kiểm tra socket và RAM', 'pcgear-store'), 'url' => $guide_ram_url),
						array('label' => __('Ước tính công suất nguồn', 'pcgear-store'), 'url' => $guide_psu_url),
					),
				),
				array(
					'type'  => 'list',
					'title' => __('WooCommerce', 'pcgear-store'),
					'links' => array(
						array('label' => __('Xem danh mục sản phẩm', 'pcgear-store'), 'url' => $shop_url),
						array('label' => __('Giỏ hàng và thanh toán', 'pcgear-store'), 'url' => $guide_checkout_url),
					),
				),
				array(
					'type'  => 'feature',
					'title' => __('Flow demo đề xuất', 'pcgear-store'),
					'text'  => __('Vào cửa hàng, mở sản phẩm, chuyển sang PC Builder, kiểm tra tương thích rồi thêm cả cấu hình vào giỏ.', 'pcgear-store'),
					'link'  => $guide_flow_url,
					'cta'   => __('Xem flow', 'pcgear-store'),
				),
			),
		),
		array(
			...
			'url'    => $software_intro_url,
			'groups' => array(
				array(
					'type'  => 'list',
					'title' => __('PC Builder', 'pcgear-store'),
					'links' => array(
						array('label' => __('Mở trình dựng cấu hình', 'pcgear-store'), 'url' => $software_intro_url),
						array('label' => __('Kiểm tra tương thích', 'pcgear-store'), 'url' => $software_rules_url),
						array('label' => __('Thêm build vào giỏ hàng', 'pcgear-store'), 'url' => $builder_url),
					),
				),
				array(
					'type'  => 'feature',
					'title' => __('WordPress + WooCommerce + plugin riêng', 'pcgear-store'),
					'text'  => __('Phần kỹ thuật cốt lõi của dự án được gom vào một hệ thống gọn, dễ demo và dễ giải thích.', 'pcgear-store'),
					'link'  => $software_intro_url,
					'cta'   => __('Xem module', 'pcgear-store'),
				),
			),
		),
	);
